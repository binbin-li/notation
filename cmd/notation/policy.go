...
func policyAddCommand(opts *policyOpts) *cobra.Command {
	...
	command := &cobra.Command{
		Use:   "add [<json_file_path>|<policy_name>]",
...
func runDeleteCommand(cmd *cobra.Command, policyNames []string) error {
	...
	if err = policyDocument.DeletePolicies(policyNames); err != nil {
		return err
	}
...
func updatePolicy(cmd *cobra.Command, opts *policyOpts) error {
	...
	if err = policyDocument.UpdatePolicies(policies); err != nil {
		return err
	}

	ioutil.PrintPolicyNames(os.Stdout, policies, "Updated Policies:")
	return nil
}
...
func addPolicy(cmd *cobra.Command, opts *policyOpts) error {
	...
	if err = policyDocument.AddPolicies(policies); err != nil {
		return err
	}
...
func (opts *policyOpts) ApplyFlags(fs *pflag.FlagSet) {
	...
	fs.StringSliceVar(&opts.override, "override", nil, "config of custom verification level")
